...
def search(instance,dead_lib,search_lib,all_lib):
    ...
    if dead_lib.Inlib(instance.mode):   # in if search lib, then stop, leave it.
        dead_lib.AddItem(instance)
        search_lib.DeleteItem(instance.mode)
        if all_lib.Inlib(instance.mode):  # if the dead in in all lib, delete it to save time
...
#        thisblockold=1
        for direction in [1,-1]:
            ...
            if dead_lib.Inlib(newmode):   # in if search lib, then stop, leave it.
                pass
            else:
                new=state(instance,newmode)
                if search_lib.Inlib(newmode):
